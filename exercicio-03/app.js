...
let quantityOfItems = dogNames.lenght

// console.log(dogNames.length - 1)
...
const initials = `${dogNames[0].slice(0, 1)}${dogNames[1].slice(
  0,
  1
)}${dogNames[2].slice(0, 1)}${dogNames[3].slice(0, 1)}${dogNames[4].slice(
  0,
  1
)}`
...
*/
const dessert = `${dogNames[0].slice(3)}${dogNames[1].slice(2)}${dogNames[4]
  .slice(2)
  .replace('n', 'd')}`
// console.log(dessert)
/*
